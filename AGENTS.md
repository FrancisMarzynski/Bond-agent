...
- Plans 01-03 of internal deployment hardening are complete: trusted proxy header contract, frontend gateway/auth, same-origin `/api/*` proxy preserving SSE in `standalone`, non-root backend runtime with durable model cache, compose healthchecks/internal override, and canonical Next `standalone` asset layout are all implemented and were live-validated through fresh Author/Shadow runs on the authenticated public frontend.
- Repo should now be treated as internal production ready; threshold/telemetry sampling stays deferred.
