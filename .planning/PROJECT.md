...
- [x] **Post-v1 Author input/recovery hardening — 2026-04-29.** Web path Author normalizuje teraz briefy `Temat / Słowa kluczowe / Wymagania`, writer wykorzystuje szczegółowy raport walidacji do ukierunkowanych retry, `checkpoint_2` oraz `/history` surfacują `draft_validation_details`, a reload + reject recovery dla Shadow / `checkpoint_1` / `checkpoint_2` ma dedykowany zielony harness Playwright (`/tmp/bond-playwright-checkpoint-recovery-20260429-115336/summary.json`).
...
- [ ] **Author draft validation quality follow-up.** Curated harness `.planning/artifacts/author-quality-20260429-113845/summary.{md,json}` potwierdził poprawną normalizację briefów i lepszą obserwowalność retry, ale nadal zakończył 4/4 case'y jako `pass_with_warnings`; następny krok to poprawa `word_count_ok`, `meta_desc_length_ok` i stabilności `keyword_in_first_para` bez cofania nowego kontraktu recovery.
- [ ] **Threshold/telemetry follow-up pozostaje odroczony.** Większa próbka opublikowanych tematów i telemetryczny feedback wracają dopiero wtedy, gdy priorytet produktu świadomie wróci do tego tematu albo gdy zamknie się pilniejszy follow-up jakościowy dla Author.
...
---
*Last updated: 2026-04-29 after landing Author brief normalization, writer auto-repair, checkpoint recovery proof, and the first curated Author-quality eval*
