...
- Domyślny parser wzbogacono o rygorystyczne bezpieczne parsowanie blokiem `try-catch` na poziomie weryfikacji struktury formatowanej `JSON.parse(data)` wzbogaconej o sprawdzenie typu obiektu `if (!parsed || typeof parsed !== "object")`. Wadliwe ładowania struktur wyrzucają teraz tylko łagodnego, deweloperskiego loga zamiennie do wywoływania crasha aplikacji.
