...
Zapoczątkowano i połączono wszystko w asynchronicznym skrypcie sterującym konsumowaniem strumienia `stream` i `resume` (przywracanie pauzy). Odpowiada we współpracy z `chatStore.ts` nad iteracyjnym przesyłem (fetch) na odpowiednie porty FastAPI lokalnie wysyłając JSON konwersacyjny.
**Dodano poprawki stabilności:**
- Wprowadzono **wzajemne blokowanie (mutex)** za pomocą singletonowej zmiennej trzymającej instancję `activeController` oraz klauzuli chroniącej `if (store.isStreaming) return;` na początku funkcji. Gwarantuje to absolutnie zakazane równoczesne uruchomienie dwóch wywołań na raz i nadpisywanie swoich zdarzeń. Zablokowany skrypt wysyła ostrzeżenie do konsoli `Stream is already active. Blocking [resumeStream/startStream]`.
- Domyślny parser wzbogacono o **rygorystyczną walidację bazującą na schematach biblioteki `zod`**. Zapobiega to uszkodzeniom drzewa DOM podczas błędnego działania backendu wyłapując ewentualne wadliwe kształty JSON przychodzących komunikatów (np. błąd formatu ID wątku, czy niedostarczenie flag decyzyjnych). Wadliwe paczki są łagodnie omijane zwracając logi (zamiast wysypywać aplikację).
- Dodano ucieczkę żądań HTTP za pociągniem klasy **`AbortController`** oraz odblokowano dostęp komponentów do niej poza plikiem poprzez opublikowanie i wyeksportowanie funkcji `export function stopStream()`. Przy wystąpieniu kolejnego żądania następuje sygnał `.abort()`, przetrwaniem pętli zapobiegając ucieczkom pamięci w starszych rurociągach. Obejmuje potężny blok `try...finally { reader.releaseLock() }` dla ostatecznego zapobiegania przepełniania buforów (memory leak). 
