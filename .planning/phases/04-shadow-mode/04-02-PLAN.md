...
npx tsc --noEmit 2>&1 | grep -E "ShadowPanel|AnnotationList|useSyncScroll" || echo "No TS errors in Shadow components"

# Verify component files exist
ls src/components/ShadowPanel.tsx src/components/AnnotationList.tsx src/hooks/useSyncScroll.ts
```
react-diff-viewer-continued is at version 4.1.2. New component files exist at expected paths. TypeScript check passes for Shadow components (or shows only unrelated pre-existing errors).
  </verify>
  <done>react-diff-viewer-continued@4.1.2 installed in frontend/package.json. ShadowPanel.tsx renders side-by-side diff viewer (read-only) plus editable textarea for corrected version with "Copy corrected" button. useSyncScroll.ts synchronizes scroll with selector guard warning. AnnotationList.tsx renders per-annotation checkboxes with status highlighting, rejection feedback textarea, and Approve/Regenerate buttons that call /api/chat/resume.</done>
</task>
