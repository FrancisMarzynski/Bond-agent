...
must_haves:
  truths:
    ...
    - "If the RAG corpus contains fewer than 10 articles, writer_node pauses via interrupt() with a low-corpus warning before generating the draft; the user must confirm (proceed=True) to continue or abort (proceed=False)"
  ...
  artifacts:
    ...
    - path: "bond/graph/nodes/writer.py"
      provides: "writer_node function: corpus count check with interrupt() on low corpus (< 10 articles), SEO-compliant draft generation with RAG few-shot injection and constraint validation with 2-retry loop"
      exports: ["writer_node", "_validate_draft", "_build_writer_prompt"]
  key_links:
    - from: "bond/graph/nodes/writer.py"
      to: "bond/store/chroma.py"
      via: "get_corpus_collection().query() for RAG exemplar fragments; get_corpus_collection().count() for low-corpus check"
      pattern: "get_corpus_collection|get_or_create_corpus_collection"
    - from: "bond/graph/nodes/writer.py"
      to: "langgraph.types.interrupt"
      via: "interrupt() call when corpus count < 10 to pause and warn user before draft generation"
      pattern: "interrupt\\("
...
Purpose: The writer node is the most complex in the pipeline — it combines SEO constraint enforcement, RAG exemplar injection, low-corpus warning (locked user decision), and targeted section revision. Getting it right here avoids refactoring when Checkpoint 2 is wired in Plan 04.
Output: Three working nodes. structure_node generates an H1/H2/H3 outline. checkpoint_1_node pauses for human approval. writer_node produces a validated, RAG-stylized SEO draft, with a required corpus-count gate before generation starts.
</objective>
...
<task type="auto">
  <name>Task 2: Writer node with low-corpus gate, RAG injection, and SEO constraint validation (AUTH-05, AUTH-06, AUTH-08, AUTH-11)</name>
  <files>
    bond/graph/nodes/writer.py
    bond/graph/graph.py
  </files>
...
1. Low-corpus gate — interrupt() with warning if corpus count < 10 (locked user decision)
2. RAG few-shot injection — fetches 3-5 exemplar style fragments from Phase 1 corpus (AUTH-06)
3. SEO-compliant draft generation using DRAFT_MODEL (AUTH-05, AUTH-11)
4. Silent auto-retry (max 2) if hard constraints fail (per user decision)
5. Targeted section revision using cp2_feedback on re-runs (AUTH-08)

Key rules:
- LOW_CORPUS_THRESHOLD = 10 — if `bond_style_corpus_v1` collection count < 10, interrupt() with a warning before draft generation. The user must respond with True (proceed) or False (abort). This is a locked user decision from CONTEXT.md: "if corpus is below the 10-article threshold, warn the user and pause — proceed only if user confirms".
...
Note: Because writer_node now calls interrupt(), bond/graph/graph.py must NOT pre-mark the graph as non-interruptible. The default behavior (all nodes may interrupt) is correct — no changes needed to compilation options.
  </action>
  <verify>
```bash
cd /Users/franciszekmarzynski/Downloads/Bond-agent
...
# Verify imports
python -c "
from bond.graph.nodes.writer import writer_node, _validate_draft, _fetch_rag_exemplars, LOW_CORPUS_THRESHOLD
print('writer_node importable:', callable(writer_node))
print('LOW_CORPUS_THRESHOLD:', LOW_CORPUS_THRESHOLD)
assert LOW_CORPUS_THRESHOLD == 10, 'Threshold must be 10 per user decision'
"
...
```
  ...
  <done>writer_node, _validate_draft, _fetch_rag_exemplars all import without error. LOW_CORPUS_THRESHOLD equals 10. _validate_draft correctly detects keyword_in_h1, keyword_in_first_para. compile_graph() succeeds with writer_node registered (not a stub).</done>
</task>
...
from bond.graph.nodes.structure import structure_node
from bond.graph.nodes.checkpoint_1 import checkpoint_1_node
from bond.graph.nodes.writer import writer_node, LOW_CORPUS_THRESHOLD
from bond.graph.graph import compile_graph
graph = compile_graph()
...
print('LOW_CORPUS_THRESHOLD:', LOW_CORPUS_THRESHOLD)
assert LOW_CORPUS_THRESHOLD == 10
"
...
- writer_node imports and is registered; LOW_CORPUS_THRESHOLD = 10
- writer_node calls interrupt() with a "low_corpus" warning payload when corpus count < 10; user can proceed (True) or abort (False)
- _fetch_rag_exemplars returns list (empty if corpus not populated — no crash); corpus count check occurs before exemplar fetching attempts draft generation
...
- Low-corpus gate: threshold value, interrupt payload, user abort path
