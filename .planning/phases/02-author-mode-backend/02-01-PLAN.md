...
must_haves:
  ...
  artifacts:
    ...
    - path: "bond/config.py"
      provides: "Settings class with all Phase 2 fields: checkpoint_db_path, metadata_db_path, exa_api_key, research_model, draft_model, min_word_count, duplicate_threshold"
      contains: "checkpoint_db_path"
  key_links:
...
**3. bond/config.py — add Phase 2 settings fields**

Update `bond/config.py` to add the seven new Phase 2 settings fields by appending to the existing Settings class:
...
**4. bond/graph/graph.py — StateGraph with stub nodes and SqliteSaver**
...
**5. bond/db/__init__.py** — empty (`# bond db package`).
  </action>
  <verify>
```bash
cd /Users/franciszekmarzynski/Downloads/Bond-agent
...
Both commands print confirmation without errors.
  ...
  <done>compile_graph() returns a compiled graph with 7 nodes in .nodes. save_article_metadata() inserts a row and get_recent_articles() returns it. data/bond_checkpoints.db and data/bond_metadata.db files are created in the data/ directory. bond/config.py Settings includes all 7 new Phase 2 fields.</done>
</task>
...
- bond/config.py Settings includes checkpoint_db_path, metadata_db_path, duplicate_threshold, min_word_count, research_model, draft_model, exa_api_key fields
</success_criteria>
